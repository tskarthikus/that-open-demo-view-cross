...
class ViewHelper extends Object3D {
	public selectedTargetPosition: Vector3;
	public animating: boolean;
	public editorCamera: Camera;
	public render: WebGLRenderer;
	public handleClick: (event: MouseEvent) => boolean = () => false;
	public update: (delta: number) => Vector3 = () => new Vector3();

	private isViewHelper: boolean;
	private center: Vector3;	
	private add: any;	
	private quaternion: Quaternion;
	private updateMatrixWorld: any;	
	private setLabels: (labelX: string, labelY: string, labelZ: string) => void = () => {};
	private setLabelStyle: (font: string, color: string, radius: number) => void = () => {};
	...
	constructor( camera, domElement ) {
		...
		this.selectedTargetPosition = new Vector3();
    ...
    this.editorCamera = camera;
		...
		const options = {labelX: '', labelY: '', labelZ: '', font:'', color:'', radius:14};

		const interactiveObjects:Sprite[] = [];
		...
		const dim = 72;
		const turnRate = 2 * Math.PI * 2; // turn rate in angles per second
		...
		this.render = function ( renderer ) {
			...
			renderer.three.getViewport( viewport );
			renderer.three.setViewport( x, 0, dim, dim );

			renderer.three.render( this, orthoCamera );

			renderer.three.setViewport( viewport.x, viewport.y, viewport.z, viewport.w );
		...
		this.handleClick = function ( event ) {
			...
			if ( intersects.length > 0 ) {
				...
				this.selectedTargetPosition = prepareAnimationData( object, this.center );
		...
		function prepareAnimationData( object, focusPoint ) {
			...
			//
			const selectedTargetPosition = targetPosition.clone()
